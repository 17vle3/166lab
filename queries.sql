...
select (sum(p.on_hand)-sum(s.on_hand))+sum(s.on_hand) SumOnHand FROM part_nyc p LEFT JOIN part_sfo s ON p.part_number=s.part_number group by p.part_number having p.color=0) As Subtable;
